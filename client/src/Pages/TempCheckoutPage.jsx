...
const TempCheckoutPage = () => {
  ...
  const fetchAddresses = async (userId) => {
    try {
      const response = await fetch(
        `http://apex.3axys.com:5000/api/address?userId=${userId}`
      );
      if (response.ok) {
  ...
  const applyVoucher = async () => {
    try {
      ...
      const response = await axios.post('http://apex.3axys.com:5000/api/validate', { code: voucherCode, userId });
      const voucher = response.data;
      const discountAmount = (voucher.discount / 100) * totalPrice;
      setMaxDiscount(discountAmount);
  
      await axios.post('http://apex.3axys.com:5000/api/apply-voucher', { code: voucherCode, userId });
    } catch (error) {
  ...
  if (paymentMethod === "razorpay") {
    try {
      const razorpayResponse = await axios.post("http://apex.3axys.com:5000/orders", {
      ...
      const options = {
        ...
        handler: async (response) => {
          ...
          try {
            const response = await axios.post("http://apex.3axys.com:5000/api/orderHistory", orderData);
