...
const Account = () => {
  ...
  const fetchOrders = async (userId) => {
    try {
      const response = await fetch(`http://127.0.0.1:5000/api/orderHistory/orders/user/${userId}`);
      if (response.ok) {
        const data = await response.json();
        setOrders(data);
      } else {
  ...
  const handleDeleteAddress = async (addressId) => {
    try {
      ...
      const response = await axios.delete(`http://127.0.0.1:5000/api/address/${addressId}`, { headers });
