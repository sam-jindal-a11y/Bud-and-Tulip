...
const OrderDetail = () => {
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      try {
        const response = await axios.get(`http://127.0.0.1:5000/api/orderHistory/${id}`);
  ...
  const handleStatusChange = async () => {
    try {
      await axios.put(`http://127.0.0.1:5000/api/orderHistory/${id}/status`, {
        status: newStatus,
      });
  ...
  const fetchUserData = async (userId) => {
    try {
      const response = await axios.get(`http://127.0.0.1:5000/api/auth/users/${userId}`);
